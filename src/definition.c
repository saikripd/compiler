	...
	do
	{
		...
		if(last->numAddresses == 0)
		{
			if(words[0][0] == 'g' && words[0][1] == 'o' && words[0][2]=='t' && words[0][3] == 'o')
				...
				last->type = 9;
			else if(words[0][0] == 'n' && words[0][1] == 'o' && words[0][2]=='p')
			{
				;
			}
			else
			{
				printf("There does not exist any type for this %s\n", words[0]);
			}
		}
	...
	while(codeP != NULL)
	{
		...
		if(globals == 1)
		{
			...
			if(codeP->type == 11)
			{
				...
				else if(strcmp(codeP->gotoAddress->label, "") == 0 && labelN != 0)
				{
				...
				{
					snprintf(codeP->gotoAddress->label, 20, "main");
					printf("j main\n");
					labelN++;
				}
				else
...
//             }
		...
		else
		{
			...
			if(codeP->isBBLHead)
			{
				if(strcmp(codeP->label,"") == 0 && labelN != 0)
				{
					snprintf(codeP->label, 20, "lbl%d", ++labelN);
				}
				else if(strcmp(codeP->label,"") == 0)
				{
					snprintf(codeP->label, 20, "main");
					labelN++;
				}
			...
			switch(codeP->type)
			{
				...
				{
					if(strcmp(codeP->gotoAddress->label, "") == 0 && labelN != 0)
					{
						snprintf(codeP->gotoAddress->label, 20, "lbl%d", ++labelN);
					}
					else if(strcmp(codeP->gotoAddress->label, "") == 0)
...
void numToReg(char arr[3][10], int n[3])
{
	int i;
	for (i = 0; i < 3; ++i)
	{
		...
		else if(n[i] >= 10 && n[i] < 18)
		{
			arr[i][0] = 's';
			arr[i][1] = '0' + n[i] - 10;
			arr[i][2] = '\0';
		}
		else if(n[i] >= 18 && n[i] < 22)
		{
			arr[i][0] = 'a';
			arr[i][1] = '0' + n[i] - 18;
			arr[i][2] = '\0';
